    ...
    { title: t('createTime'), dataIndex: 'createTime', width: 170, render: (v: string) => parseTime(v) },
    {
      ...
      render: (_: any, record: any) => (
        <Space size="small">
          ...
          <HasPermi permissions={['system:post:remove']}><Popconfirm title={t('confirmDelete')} onConfirm={() => handleDelete(record)}><Button type="link" size="small" danger icon={<DeleteOutlined />}>{t('delete')}</Button></Popconfirm></HasPermi>
        </Space>
      )
    }
  ]
      ...
      {showSearch && (
        <Card style={{ marginBottom: 16 }}>
          <Form form={queryForm} layout="inline" onFinish={handleQuery}>
            <Form.Item name="postCode" label={t('postMgmt.postCode')}><Input placeholder={t('postMgmt.postCode')} allowClear /></Form.Item>
            <Form.Item name="postName" label={t('postMgmt.postName')}><Input placeholder={t('postMgmt.postName')} allowClear /></Form.Item>
      ...
      <Card>
        <div style={{ display: 'flex', marginBottom: 16 }}>
          <Space>
            ...
            <HasPermi permissions={['system:post:remove']}><Popconfirm title={t('confirmDelete')} onConfirm={() => handleDelete()} disabled={!selectedRowKeys.length}><Button type="default" danger icon={<DeleteOutlined />} disabled={!selectedRowKeys.length}>{t('delete')}</Button></Popconfirm></HasPermi>
          </Space>
