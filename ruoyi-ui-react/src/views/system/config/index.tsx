    ...
    { title: t('createTime'), dataIndex: 'createTime', width: 170, render: (v: string) => parseTime(v) },
    {
      ...
      render: (_: any, record: any) => (
        <Space size="small">
          ...
          <HasPermi permissions={['system:config:remove']}><Popconfirm title={t('confirmDelete')} onConfirm={() => handleDelete(record)}><Button type="link" size="small" danger icon={<DeleteOutlined />}>{t('delete')}</Button></Popconfirm></HasPermi>
        </Space>
      )
    }
  ]
      ...
      {showSearch && (
        <Card style={{ marginBottom: 16 }}>
          <Form form={queryForm} layout="inline" onFinish={handleQuery}>
            <Form.Item name="configName" label={t('configMgmt.configName')}><Input placeholder={t('configMgmt.configName')} allowClear /></Form.Item>
            <Form.Item name="configKey" label={t('configMgmt.configKey')}><Input placeholder={t('configMgmt.configKey')} allowClear /></Form.Item>
            <Form.Item name="configType" label={t('configMgmt.systemBuiltIn')}>
              <Select placeholder={t('configMgmt.systemBuiltIn')} allowClear style={{ width: 120 }}>{(dict.sys_yes_no || []).map((i: any) => <Select.Option key={i.value} value={i.value}>{i.label}</Select.Option>)}</Select>
            </Form.Item>
      ...
      <Card>
        <div style={{ display: 'flex', marginBottom: 16 }}>
          <Space>
            ...
            <HasPermi permissions={['system:config:remove']}><Popconfirm title={t('confirmDelete')} onConfirm={() => handleDelete()} disabled={!selectedRowKeys.length}><Button type="default" danger icon={<DeleteOutlined />} disabled={!selectedRowKeys.length}>{t('delete')}</Button></Popconfirm></HasPermi>
